import { blur, focus, testA11y, render } from '@gobob/test-utils';
...
describe('NumberInput', () => {
  it('should render correctly', () => {
    const { unmount } = render(<NumberInput label='Amount' />);
  ...
  describe('inputMode decimal', () => {
    it('should emit onChange', async () => {
      ...
      await userEvent.type(screen.getByRole('textbox', { name: /amount/i }), '.');
  ...
  describe('inputMode numeric', () => {
    it('should emit onChange', async () => {
      ...
      await userEvent.type(screen.getByRole('textbox', { name: /amount/i }), '2');
