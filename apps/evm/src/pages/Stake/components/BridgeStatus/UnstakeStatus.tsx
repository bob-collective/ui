...
import { getOngoingStakeStep } from '../../utils/status';
...
const UnstakeStatus = ({ data, isExpanded }: WithdrawStatusProps): JSX.Element => {
  ...
  const currentStep = useMemo(
    () => (data.status ? getOngoingStakeStep(data.status, data.direction) : undefined),
    [data]
  );
  ...
  if (!isExpanded) {
    switch (currentStep) {
      ...
      default:
        return <StakeStep data={data} step={currentStep} />;
    }
  }

  return (
    <Flex direction='column' flex={1} gap='s'>
      <StakeStep data={data} step='unstake' />
