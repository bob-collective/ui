...
import { createTransferWithOpReturn, getInputFromUtxoAndTransaction } from '../src/utxo';
...
describe('UTXO Tests', () => {
  it('should spend from address to create a transaction with an OP return output', async () => {
      ...
      toAddresses.map(async (toAddress) => {
        await Promise.all(
          paymentAddresses.map(async (paymentAddress) => {
            ...
            const transaction = await createTransferWithOpReturn(
              network,
              paymentAddress,
              toAddress,
              amount,
  ...
  it('should get input from an UTXO and its transaction', async () => {
    ...
    for (const test of testset) {
      const input = getInputFromUtxoAndTransaction(
        'testnet',
        test.utxo,
        test.transaction,
        test.addressType,
