...
import { StakeAlert } from './StakeAlert';
...
const BobStakeForm = ({
  ...
  const bridgeChainId = useMemo(() => (type === 'stake' ? L1_CHAIN : L2_CHAIN), [type]);
  ...
  const { selectedCurrency, selectedToken } = useMemo(() => {
    ...
    return {
      selectedToken,
      selectedCurrency: type === 'stake' ? selectedToken?.l1Currency : selectedToken?.l2Currency
    };
  }, [ticker, tokens, type]);
  ...
  const gasEstimateMutation = useMutation({
    ...
    mutationFn: async ({
      ...
      if (currencyAmount.currency.isNative) {
        ...
        const gasAmount = await (type === 'stake'
      ...
      // TODO: USDC requires approval on unstake
      if (type === 'stake') {
  ...
  const isStakingDisabled = useMemo(
    () => (type === 'stake' ? selectedToken?.l1Token.bridgeDisabled : selectedToken?.l2Token.bridgeDisabled),
    [type, selectedToken]
  );
  ...
  const {
    ...
    enabled: Boolean(!isStakingDisabled && currencyAmount && address && selectedCurrency?.isToken && type === 'stake'),
    queryFn: async () => {
      if (!messenger) {
        throw new Error('Missing messenger');
      }
  ...
  const handleChangeCurrencyAmount = (currencyAmount: CurrencyAmount<ERC20Token | Ether>, token: BridgeToken) => {
    if (type === 'stake' && chainId === L1_CHAIN && currencyAmount.currency.isToken && currencyAmount.greaterThan(0)) {
      refetchL1Allowance();
    }
  ...
  const handleChangeTicker = (currency: Currency) => {
    ...
    const selectedCurrency = type === 'stake' ? selectedToken?.l1Currency : selectedToken?.l2Currency;
  ...
  const isSubmitDisabled = isFormDisabled(form) || !messenger || isFetchingAllowance || isStakingDisabled;

  const btnLabel = isStakingDisabled
    ? 'Staking Disabled'
    : isFetchingAllowance
      ? 'Checking Allowance'
