...
const ProfileEvmWallet = ({ chainId, onPressConnect }: ProfileEvmWalletProps): JSX.Element | null => {
  ...
  const { evmWallet, btcWallet } = useDynamicWallets();
  ...
  const { hoverProps, isHovered } = useHover({ isDisabled: !evmWallet });
  ...
  return (
    <ProfileWallet
      ...
      walletAvatar={
        evmWallet && <WalletIcon style={{ height: '1rem', width: '1rem' }} walletKey={evmWallet.connector.key} />
      }
