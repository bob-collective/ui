...
const TokenTable = ({ ...props }: TokenTableProps): JSX.Element => {
  ...
  const columns = [
    { name: 'Asset', id: TokenTableColumns.ASSET },
    { name: 'Balance', id: TokenTableColumns.BALANCE },
    { name: '', id: TokenTableColumns.ACTION }
  ];

  const handlePressBridge = useCallback(
    (ticker: string) => router.push(`${RoutesPath.HOME}?ticker=${ticker}`),
  ...
  const btcRow: TokenTableRow = useMemo(() => {
    ...
    return {
      ...
      action: (
        <ButtonGroup
          connectLabel='Connect BTC Wallet'
